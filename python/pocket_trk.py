...
def read_data(fp, N, IQ, buff, ix, sdrname="pocketsdr"):
    ...
    if len(raw) < N * IQ:
        ...
        buff[ix : ix + N] = np.array(
            raw[0::2] / div + raw[1::2] * 1j * qsgn / div, dtype="complex64"
            # raw[0::2] / div - raw[1::2] * 1j * qsgn / div, dtype="complex64"
        )
    return True
...
def show_usage():
    ...
    print("       [-log path] [-q] [-sdr pocketsdr|bladerf] [file]")
    exit()
...
if __name__ == "__main__":
    ...
    log(
        3,
        "$LOG,%.3f,%s,%d,START FILE=%s FS=%.3f FI=%.3f IQ=%d TOFF=%.3f"
